...
/**
 * The public digital business card surface. One markup tree, four full-bleed
 * designs — the look is selected entirely by `card.card_theme`
 * (carddesk | glass | editorial | tech), rendered as `data-card-theme` on the
 * root and styled by the scoped CSS below.
 *
 * The glass / editorial / tech directions mirror the Greater Opportunities
 * pitch directions (frosted-dark glass, cream print Editorial with Bauer Bodoni,
 * light Linear/Vercel-docs Tech). There is NO floating card container — content
 * flows directly on the themed page, the way those directions do.
 *
 * Self-contained: it carries its own themed backdrop, so it looks identical for
...
const props = withDefaults(
  defineProps<{
    card: CardViewData
    /** Public share URL (/c/:id) used by the Share action. */
    shareUrl?: string
    /** Whether tel/mail/share actions fire. False in the account preview. */
    interactive?: boolean
    /** Show a floating "Share" button (bottom of viewport) that opens the QR. */
    floatingQr?: boolean
    /** Raise the floating button (e.g. above the preview switcher). */
    floatRaised?: boolean
  }>(),
  { shareUrl: '', interactive: true, floatingQr: false, floatRaised: false },
)

const { shareContact, shareUrl: shareLink } = useShare()
...
async function shareCard() {
  if (!props.interactive) return
  const url = props.shareUrl || (import.meta.client ? window.location.href : '')
  if (!url) return
  const res = await shareLink({ url, title: `${props.card.name} · CardDesk`, text: `Here’s ${props.card.name}’s card` })
...
// ── Cover parallax — the cover scrolls at ~half speed so the content sheet
// lifts off it as you scroll, an iOS-Wallet-like depth effect. Driven off the
// nearest scroll container (window on /c/:id, the overlay in the app). ──
const coverEl = ref<HTMLElement | null>(null)
let scrollTarget: HTMLElement | Window | null = null
let rafPending = false
function applyParallax() {
  rafPending = false
  if (!coverEl.value || !scrollTarget) return
  const s = scrollTarget === window ? window.scrollY : (scrollTarget as HTMLElement).scrollTop
  coverEl.value.style.transform = `translate3d(0, ${Math.max(0, s) * 0.65}px, 0)`
}
...
onMounted(() => {
  // Skip in the static account preview (interactive=false) — it shouldn't react
  // to the editor page scroll.
  if (!coverEl.value || !props.interactive) return
  scrollTarget = findScrollParent(coverEl.value)
  scrollTarget.addEventListener('scroll', onParallaxScroll, { passive: true })
  applyParallax()
})
onBeforeUnmount(() => {
  scrollTarget?.removeEventListener('scroll', onParallaxScroll)
})
</script>
...
<template>
  <div class="cv" :data-card-theme="theme" :class="{ 'cv--cover': !!card.coverUrl }">
    ...
    <div class="cv-main">
      ...
      <button class="cv-save" type="button" @click="saveContact">
    ...
    <!-- Floating "Share" button + full-screen QR — client-only interactive
         chrome (teleported to body; rendered after hydration to avoid SSR
         mismatch). -->
    <ClientOnly>
    <Teleport to="body">
      <button
        v-if="floatingQr"
        class="cv-fab"
        :class="{ raised: floatRaised }"
        :style="{ '--qr-accent': qrAccent }"
        :data-card-theme="theme"
        type="button"
        @click="openQr"
      >
        <CdIcon emoji="▦" icon="lucide:qr-code" :size="19" />
        <span>Share my card</span>
      </button>
    </Teleport>

    <!-- Full-screen QR — the "show my phone to someone" view. -->
    <Teleport to="body">
      <Transition name="cv-qrov">
        <div v-if="qrOpen" class="cv-qrov" :style="{ '--qr-accent': qrAccent }" :data-card-theme="theme" @click.self="qrOpen = false">
          <button class="cv-qrov-x" type="button" aria-label="Close" @click="qrOpen = false"><CdIcon emoji="×" icon="lucide:x" :size="22" /></button>
          <div class="cv-qrov-body">
            <div class="cv-qrov-name">{{ card.name }}</div>
            <div class="cv-qrov-sub"><CdIcon icon="lucide:scan-line" :size="14" /> Point a camera here to save my contact</div>
            <div class="cv-qr-box cv-qrov-box">
              <template v-if="qr">
                <img class="cv-qr-img" :src="qr" alt="Scan to save contact" />
                <i class="cv-qr-scan" aria-hidden="true"></i>
                <i class="cv-qr-corner tl" aria-hidden="true"></i>
                <i class="cv-qr-corner tr" aria-hidden="true"></i>
                <i class="cv-qr-corner bl" aria-hidden="true"></i>
                <i class="cv-qr-corner br" aria-hidden="true"></i>
              </template>
