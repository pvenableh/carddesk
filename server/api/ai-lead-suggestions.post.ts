...
export default defineEventHandler(async (event) => {
  ...
  const account = await enforceCredits(event, "ai-lead-suggestions");
...
Use relevant emoji icons. Be specific — mention numbers, contact names, and concrete actions. No generic advice.`;
  ...
  try {
    ...
    console.error("[ai-lead-suggestions] Anthropic error:", err?.status ?? err?.statusCode, detail, err);
    throw createError({ statusCode: 502, message: `AI lead suggestions failed: ${detail}` });
  }
});
